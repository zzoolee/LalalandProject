...
import lala.service.IRestaurantService;
import lala.service.RestaurantServiceImpl;
import lala.vo.RestaurantVO;
import lala.vo.attractionVO;


//web.xml에서 설정하거나 어노테이션(@WebServlet)해도 됨. 
@WebServlet(value = "/adminrestaurant.do")
public class AdminRestaurantController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//1. 서비스 생성하기
		IRestaurantService resService = RestaurantServiceImpl.getInstance();
...
//		vo.setZoneCd(zoneCd);
		...
		List<RestaurantVO> resList = resService.getRestaurant();
		...
		RequestDispatcher rd = req.getRequestDispatcher("views/enjoy/adminrestaurant.jsp");
		rd.forward(req, resp);
