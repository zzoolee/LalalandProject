...
<!-- 만약 세션아이디와  멤버아이디가 같다면  -->
	...
	<a href="<%=request.getContextPath()%>/qna/adminMain.do"><p>
			<button class="btn btn-dark">목록</button></a>
	<a href="<%=request.getContextPath()%>/qna/adminInsert.do?qCd=<%=cqv.getqCd()%>"><button
			class="btn btn-dark">답변 등록/수정</button></a>
