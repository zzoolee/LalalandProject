...
<%
	...
	String msg = session.getAttribute("msg") == null ? "" 
			...
			//만약 null이라면 빈 문자열("")을 할당하고, 그렇지 않으면 가져온 값을 그대로 할당
				...
				// 세션에서 "msg" 속성을 제거
	//String headerFile = memId != null ? "../../header.jsp" : "../../mem_header.jsp";
				...
				<table class="board-table">
					<thead>
						<tr>
							...
							<th scope="col" class="th-aChk">답변상태</th>
						</tr>
					</thead>
				<tbody>
	<%
	...
	%>
		<tr>
			...
			%>
				...
				<a href="./userdetail.do?qCd=<%=cqv.getqCd() %>"><%=cqv.getqTitle() %></a>
			<%
				} else {
			%>
				<%=cqv.getqTitle() %>
...
<p><a href="<%=request.getContextPath() %>/qna/userinsert.do"><button type="submit" class="btn btn-dark">등록</button></a></p>
...
%>
<script>
  var notAcceptLinks = document.querySelectorAll('#board-list a[href="./userdetail.do"]');
  for (var i = 0; i < notAcceptLinks.length; i++) {
    notAcceptLinks[i].addEventListener('click', function(event) {
      event.preventDefault();
      alert('상세 정보를 볼 수 있는 권한이 없습니다.');
    });
  }
</script>
